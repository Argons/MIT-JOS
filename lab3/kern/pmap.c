...
// has been setup roughly correctly(by  = boot_alloc (NENV * sizeof (struct Env), PGSIZE);
//
...
/*	int i;
	...
	for(pa=PGSIZE;pa < IOPHYSMEM;pa+=PGSIZE){
		pageptr = pa2page(pa);
		pageptr->pp_ref = 0;
	...
	physaddr_t kuplim;
	kuplim =EXTPHYSMEM+(uintptr_t)boot_freemem-KERNBASE;
	for(pa=EXTPHYSMEM;pa<kuplim;pa+=PGSIZE){
		pageptr =  pa2page(pa);
		pageptr->pp_ref = 1;
	}

	for(i=PPN(kuplim);i<npage;i++){
		pages[i].pp_ref = 0;
		LIST_INSERT_HEAD(&page_free_list, &pages[i], pp_link);
	}*/
/*	int i, phy_add;
