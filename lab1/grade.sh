...
runbochs () {
	# Find the address of the bochs.out readline function,
	# which the bochs.out monitor uses to read commands interactively.
	brkaddr=`grep 'readline$' bochs.out | sed -e's/ .*$//g'`
	#brkaddr=`grep 'readline$' obj/kern/bochs.out.sym | sed -e's/ .*$//g'`
